...
export async function createProject(targetDirectory, images, ci = false, enableMongoAuth = false, cloud = false) {
    ...
    if (targetDirectory) {
        ...
        const message = `${chalk.red('ERROR:')} the directory ${chalk.blueBright.bold(targetDirectory)} already exists. Run ${chalk.cyan.bold('ecapsvc init')} again and choose another directory.`
